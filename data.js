// Artist data module
export const artists = {
    executives: [
        {
            name: 'nyx',
            followers: '12K',
            achievements: [
                'Meow music asia主理人',
                'Suno總排行(Popular Users)中排行第七'
            ],
            works: [
                { title: 'Mad Mad World', url: 'https://suno.com/s/Cdambhe8oJudA0WX' },
                { title: 'Don\'t Care Anymore', url: 'https://www.youtube.com/watch?v=JgbFql5XNpQ' }
            ]
        },
        {
            name: 'ChillVibeCircuits',
            followers: '767',
            achievements: [
                'SSC5半決賽',
                'SSC6半決賽'
            ],
            works: [
                { title: 'Beyond the Screen', url: 'https://suno.com/song/7064a52a-ae66-4649-b5f9-7b7ced82ea52' },
                { title: '红尘，仙缘', url: 'https://youtu.be/qjcKJZZWqQE?feature=shared' }
            ]
        },
        {
            name: '裴芸PuiWan',
            followers: '954',
            achievements: [],
            works: [
                { title: '孤海 (V2)', url: 'https://suno.com/song/b3ef97c8-1a41-4ff5-8337-47472ef65f06' },
                { title: '白骨洞', url: 'https://suno.com/song/3a0b7724-0c8e-4d70-8579-a57cf223e716' }
            ]
        },
        {
            name: '🔱AL ren',
            followers: '840',
            achievements: [],
            works: [
                { title: '清麗如卿 | My Teacher', url: 'https://suno.com/song/cef75013-0097-46ab-8101-355ceeac205b' },
                { title: '老朋友 | Old Friend', url: 'https://suno.com/song/25e778f7-2731-4e0c-b681-ccd4eb370a2b' }
            ]
        },
        {
            name: 'GameIsHere🐢',
            followers: '2.6K',
            achievements: [
                'SSC5半決賽',
                'SSC6總決賽第19名',
                'Testing Ground主理人',
                'AIMAC中一員',
                '爆肝做這個網站的人:)'
            ],
            works: [
                { title: 'Space', url: 'https://suno.com/song/f434929e-a6b2-4437-a597-45ffece514b1' },
                { title: 'The Gaze Pt.II', url: 'https://suno.com/s/cdcyIVOjHHI7yUCH' }
            ]
        },
        {
            name: 'Wine loves flowers',
            followers: '3.8K',
            achievements: [
                'SSC5半決賽',
                'SSC6總決賽第14名'
            ],
            works: [
                { title: 'I am separated from the world✨', url: 'https://suno.com/song/682349a7-fc02-46f1-8fbd-aed67dee2c6e' },
                { title: 'daily社畜✨', url: 'https://suno.com/s/dre1IgGHt2UZHVBo' }
            ]
        },
        {
            name: '桂花巷子/Osmanthus Alley',
            followers: '1.6K',
            achievements: [
                'SSC5半決賽',
                'SSC6半決賽'
            ],
            works: [
                { title: 'Kung Fu鴨', url: 'https://suno.com/song/20e4acb5-a9ec-43ab-bf0a-41a35db63686' },
                { title: '山の魂', url: 'https://suno.com/song/142b1d6e-b6c0-455b-a830-68445c6b1990' }
            ]
        },
        {
            name: 'MidnightHour',
            followers: '404',
            achievements: [],
            works: [
                { title: 'Poutine Pudding #TTChallenge', url: 'https://suno.com/song/560eed74-88b2-4c6d-bbde-1dbb11bf075e' },
                { title: 'Y.B.B.C. #coolerInMay', url: 'https://suno.com/song/045861e5-5ce6-495f-a2f6-fe42d0582e6f' }
            ]
        },
        {
            name: 'iLoveSuno',
            followers: '2.1K',
            achievements: [
                'AI音樂元老'
            ],
            works: [
                { title: '醉打蔣門神', url: 'https://suno.com/song/f5a86119-2283-4e9a-86ae-9c96476f1450' },
                { title: '宮保雞丁', url: 'https://suno.com/s/fMacjQlMrRm2QFSZ' }
            ]
        },
        {
            name: '二月February',
            followers: '6.5K',
            achievements: [
                'AI音樂元老',
                'SSC5半決賽',
                'SSC6半決賽'
            ],
            works: [
                { title: '醉舞江湖', url: 'https://suno.com/song/c6bc45aa-db4e-4c47-a0f9-8f9e865cc9c0' },
                { title: 'PLAY', url: 'https://suno.com/song/b0e54323-24b3-4be0-aec8-ac9156a95f45' }
            ]
        },
        {
            name: '𝓦𝓪𝓵𝓴𝓲𝓷𝓰 𝓦𝓲𝓽𝓱 𝓐𝓘(Walking With Aida)',
            followers: '2.2K',
            achievements: [],
            works: [
                { title: 'Bridges of Dreams', url: 'https://suno.com/song/4cabb878-c65c-4a0d-ad60-94fb2cfaa0a0' },
                { title: '唔好惹我 (Don\'t Mess with Me)', url: 'https://suno.com/song/82302466-23a1-4187-94f2-548bbfc063a2' }
            ]
        },
        {
            name: '豬羊',
            followers: '337',
            achievements: [],
            works: [
                { title: '遲來的話', url: 'https://suno.com/song/c3b57d85-ae83-42fb-861d-840e125ea4e1' },
                { title: '一杯琴酒', url: 'https://suno.com/song/5e0f50d0-5814-4597-9766-e3d77a356dc8' }
            ]
        },
        {
            name: '麥客官(MicroGuan)',
            followers: '438',
            achievements: [],
            works: [
                { title: '等一個等不到的人', url: 'https://suno.com/song/b704e2cb-20d0-4c40-96ee-4251c59506ec' },
                { title: '訂閱接連漲價！', url: 'https://suno.com/s/aSudikua0NloBshG' }
            ]
        },
        {
            name: 'GAZERIEL (阿明来哉)',
            followers: '132',
            achievements: [],
            works: [
                { title: 'Soundclash Nirvana', url: 'https://suno.com/song/65a8ce46-acd1-4f93-83ac-786ea8ab3512' },
                { title: '山歌核(C-Folkcore)', url: 'https://suno.com/song/e92250b5-1f07-41cb-93a0-632e8193cee4' }
            ]
        },
        {
            name: 'Brutus',
            followers: '11K',
            achievements: [
                'AI音樂元老'
            ],
            works: [
                { title: 'Under the Moon', url: 'https://suno.com/song/04440f12-078f-434c-837e-4fac9990713d' },
                { title: 'Lingering Shades of Rain', url: 'https://suno.com/song/1dc71ed6-4d3c-43ab-b864-ccdaf7700ad7' }
            ]
        },
        {
            name: 'Coreone286',
            followers: '1.1K',
            achievements: [],
            works: [
                { title: '愛不簡單 Harmony', url: 'https://suno.com/s/KqzYHkolgIwxhkdk' },
                { title: '愛妳 𝔢𝔱𝔢𝔯𝔫𝔦𝔱𝔶', url: 'https://suno.com/song/86d3e71e-5947-4e5e-a7e1-4a0437435838' }
            ]
        }
    ],
    members: [
        {
            name: 'EricWei',
            followers: '179',
            achievements: [],
            works: [
                { title: '如果是勇者欣梅爾的話，他一定也會這麼做', url: 'https://suno.com/song/afc96bf5-a315-4dd0-861e-1428b0f1e96d' },
                { title: '什麼都無法捨棄的人，什麼也改變不了', url: 'https://suno.com/song/5815f764-6f72-4606-90d8-089e3d4dd06c' }
            ]
        },
        {
            name: '海洋# Ocean',
            followers: '217',
            achievements: [],
            works: [
                { title: '"Lambada Style" Fall in love with blonde beauty #戀上金髮美女', url: 'https://suno.com/s/IztcVLsCe3frgJKF' },
                { title: '一個人的美', url: 'https://suno.com/song/8f5d6c6f-ab05-43eb-a33a-2fd65c17aa4c' }
            ]
        },
        {
            name: '雪國',
            followers: '25',
            achievements: [],
            works: [
                { title: '瀟灑人間', url: 'https://streetvoice.com/Cecilia_Cheung/songs/795333/' },
                { title: '讓靈魂自由一下', url: 'https://streetvoice.com/Cecilia_Cheung/songs/801487/' },
                { title: '小女巫的不可思議生活', url: 'https://streetvoice.com/Cecilia_Cheung/songs/812956/' }
            ]
        },
        {
            name: 'gues0530(喵吉拉)',
            followers: '18',
            achievements: [],
            works: [
                { title: '婚紗', url: 'https://suno.com/song/bc7db176-d118-4b01-bf35-84b5a40b0b4c' },
                { title: '霸王卸甲', url: 'https://suno.com/song/27dcd2a7-4f46-492a-8dc1-56d043e6bb64' }
            ]
        },
        {
            name: '閱微草堂（Little Wisdom Hut）',
            followers: '20',
            achievements: [],
            works: [
                { title: 'New Journey-嶄新的旅程', url: 'https://suno.com/s/TSgBgGPv7utxl2V5' },
                { title: 'Whispering Sunset Glow-晚霞輕語', url: 'https://suno.com/song/5feb18cc-e2ef-4ab6-a43c-125408e2ca99' }
            ]
        },
        {
            name: 'Wizardx07',
            followers: '728',
            achievements: [],
            works: [
                { title: '蒼穹の約束', url: 'https://suno.com/song/0d824e3d-ad0f-4b4f-96e3-a1889beb9fbe' },
                { title: '永遠の愛', url: 'https://suno.com/song/27454946-b61a-4567-934c-3fabc55fb248' }
            ]
        },
        {
            name: '柒',
            followers: '29',
            achievements: [],
            works: [
                { title: '警察与万圣节', url: 'https://suno.com/s/Z9UCCtvTTNpGzWNS' },
                { title: '明月几时有', url: 'https://suno.com/song/778c30f9-b4a4-4747-b778-73a4a21f8400' }
            ]
        },
        {
            name: 'Cyber Paladin',
            followers: '4',
            achievements: [],
            works: [
                { title: 'Yours and Not Yours', url: 'https://suno.com/song/7a4e47fa-8072-42ed-bf6e-d1470bdfd233' },
                { title: 'Tiny Paw Fight', url: 'https://suno.com/song/2f9c7434-400b-4ade-99e5-44a7dcf76503' }
            ]
        },
        {
            name: 'POE3030',
            followers: '31',
            achievements: [],
            works: [
                { title: '沒有歌詞可供賞析', url: 'https://suno.com/song/fcab380b-164d-4eda-9100-692f816e23e4' },
                { title: '歌謠', url: 'https://suno.com/song/5994ab40-b332-46db-86fd-dc1eb0504b42' }
            ]
        },
        {
            name: 'spearfalcon730224',
            followers: '43',
            achievements: [],
            works: [
                { title: '戲子-情愛詛咒之唱', url: 'https://suno.com/song/bf184e7a-21f8-4b0a-87c8-b363315c3c2e' },
                { title: 'spirit and sword of the Disaster - fate and approaching', url: 'https://suno.com/s/lMIp03g1WHjJBacP' }
            ]
        },
        {
            name: 'kaixin992025',
            followers: '21',
            achievements: [],
            works: [
                { title: 'Hey 给你最后的一封情书', url: 'https://suno.com/song/85e7e1a7-afea-42d1-996d-4491c83aa11e' },
                { title: '美麗的误会', url: 'https://suno.com/s/axsnGhD6HYjT8QSQ' }
            ]
        },
        {
            name: 'Leader One',
            followers: '91',
            achievements: [],
            works: [
                { title: 'Roaring Across the Horizon (remix by Leader One)', url: 'https://suno.com/song/1d3fca6a-39ec-4cfb-9ea7-11af8fe7c1e8' },
                { title: 'Panopticon', url: 'https://suno.com/s/rQ3eT6NPPoNhUoy4' }
            ]
        },
        {
            name: 'kingdan',
            followers: '14',
            achievements: [],
            works: [
                { title: '柴米油鹽V1', url: 'https://suno.com/s/N3eGLJE1BQi2F4eR' },
                { title: '憂傷版 - 想念！是一生一世的重量。', url: 'https://suno.com/s/s04aRtztI3jKSiR0' }
            ]
        },
        {
            name: 'Joey Joo',
            followers: '17',
            achievements: [],
            works: [
                { title: '慢慢地喜欢你', url: 'https://suno.com/s/zNR7jXBrjOY9N5WT' },
                { title: '记忆的尽头，你还记得我吗？', url: 'https://suno.com/s/OhWCBtO7YppP5DTZ' }
            ]
        },
        {
            name: 'HOK',
            followers: '107',
            achievements: [],
            works: [
                { title: 'meow', url: 'https://suno.com/s/jc7rr5g6KDmghovY' },
                { title: '夜的妝', url: 'https://suno.com/s/OPpE6O0799iT8KSq' }
            ]
        },
        {
            name: 'danta2353',
            followers: '17',
            achievements: [],
            works: [
                { title: '若南风知我情深', url: 'https://suno.com/song/657161af-2c51-4d4f-89c7-c8d18427de54' },
                { title: '不候', url: 'https://suno.com/s/5N0aETf9LYyvahZM' }
            ]
        },
        {
            name: 'Rastvas',
            followers: '222',
            achievements: [],
            works: [
                { title: 'Creyst', url: 'https://suno.com/s/Bxgxwgg1mnUxrw42' },
                { title: 'No.23 Pip-pop', url: 'https://suno.com/song/fd5a7e09-80a0-41fe-85ca-429ec60cea3a' }
            ]
        },
        {
            name: 'FuturEvoLab',
            followers: '1.4K',
            achievements: [],
            works: [
                { title: '🐱Cat｜猫', url: 'https://suno.com/s/0uzZygXNIWm6UWwr' },
                { title: '🎭Yes? No?', url: 'https://suno.com/s/FMqr9usjHUBpX3lx' }
            ]
        },
        {
            name: '起手氏Begin Clan',
            followers: '332',
            achievements: [],
            works: [
                { title: '🎶Power of the Stars🎶', url: 'https://suno.com/song/fd25ff68-6cf2-4b3e-8d67-4a9d4d6db6fa' },
                { title: '🎶傾聽的溫柔🎶', url: 'https://suno.com/s/8OYjlOvjKrGQYYdf' }
            ]
        },
        {
            name: 'Jane Ford',
            followers: '205',
            achievements: [],
            works: [
                { title: 'Голос берёзового леса（白樺林的風聲)', url: 'https://suno.com/s/dqzOqtKFuveDY9KD' },
                { title: 'Hymns-Good Morning, Lord', url: 'https://suno.com/s/iCPEoExxukumv4dk' }
            ]
        },
        {
            name: 'UncleMartin',
            followers: '64',
            achievements: [],
            works: [
                { title: '給我', url: 'https://suno.com/song/2ba63b4a-ed5a-42ec-ab1d-67eee9ebc67a' },
                { title: '沒妳，很好', url: 'https://suno.com/song/19c62d2e-5f0a-4820-9464-9c1ac938eed7' }
            ]
        },
        {
            name: 'PopKoi',
            followers: '28',
            achievements: [],
            works: [
                { title: '🍄 灵感菇', url: 'https://suno.com/s/moSXqLWce461sG1a' },
                { title: '♾️ 平行时空说再见', url: 'https://suno.com/s/bPDqZR70lxml78Oo' }
            ]
        },
        {
            name: 'Galvaniz',
            followers: '50',
            achievements: [],
            works: [
                { title: '那個下雨後的週六', url: 'https://suno.com/s/AFEZqf1VelSfraQt' },
                { title: '火車站旁的滷肉飯', url: 'https://suno.com/s/2ntJGy4RjZlHHfd9' }
            ]
        },
        {
            name: 'Silamos',
            followers: '100',
            achievements: [],
            works: [
                { title: 'Tribe meets AI', url: 'https://suno.com/song/ca5d9c1b-8d22-4cd2-8402-7e0a37886018' },
                { title: 'Blossoms of the Sun', url: 'https://suno.com/s/CHj9oskdQheVRucx' }
            ]
        }
    ]
};